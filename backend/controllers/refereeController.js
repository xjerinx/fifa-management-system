...
exports.getAll = async (req, res, next) => {
    try {
        ...
        try {
            ...
            console.warn('[refereeController.getAll] Falling back to referee query:', tableErr.message);
            const [fallback] = await db.query(`SELECT r.*, 0 AS matches_officiated FROM referee r ORDER BY r.last_name`);
            rows = fallback;
        }
...
exports.getMatches = async (req, res, next) => {
    try {
        ...
        try {
            ...
            console.warn('[refereeController.getMatches] Error querying match_referees:', tableErr.message);
            rows = [];
        }
